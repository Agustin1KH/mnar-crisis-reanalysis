...
\title{Selection-Robustness of the {INCOME} Effect on Crisis Outcomes:\\
       An MNAR Re-analysis of \citet{metrick2024}}
\author{Agust\'in Kupfer\thanks{All numerical claims in this memo are
  ...
  Correspondence: \texttt{agustind.kupferh@\,...}}}
\date{April 2026}
...
Laeven--Valencia, Bordo--Vassard--Xenofontos) list crisis $i$, and ranges
over $\{0, 1, 2, 3, 4\}$. The argument for excludability is that
...
Three patterns are immediate. First, on the four outcomes where the
paper's logit is statistically distinguishable from zero
(\texttt{guarantees\_d}, \texttt{lending\_d},
...
imputation-recovered estimates. Second, on \texttt{guarantees\_d} and
...
precision-overstated relative to what either the imputation or the
partial-identification approach is willing to support.
